package Interface.Raspberry_JV.Set_DI_RPI_JV;
...
public class Set_DI_RPI_JV extends JVSMain
{
  private Image image;
  String s = "";
  ...
  final String Element_Tag= "#Element:|Set_DI_RPI_JV|";
  ...
  VSBoolean Pull_Resistor = new VSBoolean(false);
  ...
  String Pull_Res_Mode ="in"; // gpio mode <pin> in/out/pwm/clock/up/down/tri
  ...
  public void init()
  {
    ...
    element.jSetPinDescription(7, "Pull_UP(T)_Pull_Down(F)_in");
    ...
    { 
      ...
      try {
          ...
          //element.jConsolePrintln();
                    ...
                    sysOut_N_Err="";
                    sysOut_Err="";
                    if(Pull_Resistor.getValue()==true){
                         Pull_Res_Mode="up";
                        }else{
                        Pull_Res_Mode="down";
                    }
                    Process p = Runtime.getRuntime().exec("gpio mode " + wPi_Pin_Number_in.getValue() + " " +Pull_Res_Mode ); 
                    if (Debug_Window_En_in.getValue()) {      
                        element.jConsolePrintln(Element_Tag+Debug_Tag+"|Executing:"+"gpio mode " + wPi_Pin_Number_in.getValue() + " " +Pull_Res_Mode +"|");
                        }
                    ...
                    if (Debug_Window_En_in.getValue()) {
                        element.jConsolePrintln(Element_Tag+Error_Tag+"|Command Error: gpio mode " + wPi_Pin_Number_in.getValue() + " " +Pull_Res_Mode+"|");
                    }
                    Error_out.setValue(true);
