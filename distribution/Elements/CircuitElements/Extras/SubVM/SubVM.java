...
public class SubVM extends JVSMain
{
  ...
  // Notwendig f�r die doc.html & ect...
  ...
  public void xOnInit()
  {
    ...
    setPin(rightC,element.C_BOOLEAN,element.PIN_INPUT);
    element.jSetPinDescription(rightC,"Show");

    for (int i=0;i<inputs.length;i++)
    {
      dt=inputs[i].jGetPinDataType(0); // Pin hat nur ein Pin!
      element.jSetPinDescription(rightC+i+1,inputs[i].jGetCaption());
      setPin(rightC+i+1,dt,element.PIN_INPUT);
    }
  ...
  public void initInputPins()
  {
    ...
    for (int i=0;i<inputs.length;i++)
    {
      in[i]=(VSObject)element.getPinInputReference(outputs.length+i+1);
    }
    inShow=(VSBoolean)element.getPinInputReference(outputs.length);
  }
  ...
  public void initOutputPins()
  {
    ...
    for (int i=0;i<inputs.length;i++)
    {
      outX[i]=element.jCreatePinDataType(outputs.length+i+1);
  ...
  public void elementActionPerformed(ElementActionEvent evt)
  {
      ...
      if (evt.getSourcePinIndex()>=outputs.length)
      {
        ...
        int index = evt.getSourcePinIndex()-outputs.length-1;
        if (index>=0)
        {
  ...
  {
    element.jAddPEItem("Modal",modal, 0,0);
  }
