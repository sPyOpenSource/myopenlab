...
public class TP extends JVSMain
{
   ...
   public void paint(java.awt.Graphics g)
   {
    if (element!=null)
    {
       ...
       //g.drawRect(bounds.x,bounds.y,bounds.width,bounds.height);
         ...
         p1 = new Polygon();
         p1.addPoint(mx,bounds.y);
         p1.addPoint(mx,my);
         p1.addPoint(bounds.x+bounds.width,my);

         p2 = new Polygon();
         p2.addPoint(mx,my);
         p2.addPoint(mx,bounds.y+bounds.height);
         p2.addPoint(bounds.x,my);
  ...
  public void elementActionPerformed(ElementActionEvent evt)
  {
    ...
    if (basis!=null) basis.vsAddTestpointValue(element,vv);
    
    int io;
    
    if (nodeDT==ExternalIF.C_BOOLEAN)
    {
      // Wenn ein Pin True ist m�ssen alle Ausg�nge True sein!
