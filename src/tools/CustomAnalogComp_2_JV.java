...
public class CustomAnalogComp_2_JV extends JVSMain
{
  ...
  public void drawAnzeige(java.awt.Graphics gx,int x, int y, int w, int h)
  {
      ...
      int Tam=((3*TempW/4)-15)+abstand.getValue(); // Tamao del circulo cuando se selecciona Texto Externo
      if(textInside.getValue()){
      TempW=(int)((w-distance*2)-InsideGapW)-abstand.getValue();
      TempH=(int)((h-distance*2)-InsideGapH)-abstand.getValue();
      Tam=((3*TempW/4)-15)+abstand.getValue(); // Tamao del circulo cuando se selecciona Texto Externo    
      }
      ...
      g.setColor(lineColor.getValue());
      if (showText.getValue()) // Dibujar Arco sobre el que van las lneas de divisin
      { 
      ...
      { 
        ...
        double vectorLaenge=((Math.sqrt((x1*x1)+(y1*y1))-(double)d2)-(InsideGapW/2)+3); // Vector sobre el que quedarn las rayitas polares del grfico
        ...
        if (showText.getValue())
        {
          ...
          double vectLen1=vectorLaenge+(double)d2; // Tamao rayas indicadoras
  ...
  public void setPropertyEditor()
  {
    ...
    element.jAddPEItem("�u�ere Kante",SetKnobStroke, 0,0);
    element.jAddPEItem("�u�ere Breite",borderStroke, 0,50);
