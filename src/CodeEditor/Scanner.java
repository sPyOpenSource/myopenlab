    ...
    while(true)
    {
      ...
      switch (state)
      {
        case STATE_INIT:
          switch (lookahead)
          {
            case '\'':state = STATE_STRING;token.start = pos; token.id = TOKEN_STRING; next(); break;
            case '"':state = STATE_QUOTED_ID;token.start = pos; token.id = TOKEN_QUOTED_ID; next(); break;
